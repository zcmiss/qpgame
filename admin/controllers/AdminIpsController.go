...
var adminIps = models.AdminIps{} //模型

type AdminIpsController struct{}

/**
 * @api {get} admin/api/auth/v1/admin_ips 登录IP授权列表
 * @apiDescription
 * <span style="color:lightcoral;">接口负责人: juzi</span><br/><br/>
 * <strong>IP记录列表</strong><br />
 * 业务描述: IP记录列表</br>
 * @apiVersion 1.0.0
 * @apiName     indexAdminIps
 * @apiGroup    admin
 * @apiPermission PC客户端
 ...
 * @apiSuccess (data-rows每个子对象字段说明) {string}     ip                      可访问的ip
 *
 ...
 * }
 */
func (self *AdminIpsController) Index(ctx *Context) {
	index(ctx, &adminIps)
}

/**
 * @api {get} admin/api/auth/v1/admin_ips/delete 登录IP授权删除
 * @apiDescription
 * <span style="color:lightcoral;">接口负责人: juzi</span><br/><br/>
 * <strong>IP记录删除</strong><br />
 * 业务描述: IP记录删除</br>
 * @apiVersion 1.0.0
 * @apiName     deleteAdminIps
 * @apiGroup    admin
 * @apiPermission PC客户端
 * @apiHeader (请求头) {string} Authorization 用户令牌  格式为 token
 * @apiHeaderExample {json} 请求头示例
 ...
 * }
 */
func (self *AdminIpsController) Delete(ctx *Context) {
	remove(ctx, &adminIps)
}
